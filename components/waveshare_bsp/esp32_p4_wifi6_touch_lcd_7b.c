...
esp_err_t bsp_display_panel_recover(void)
{
    if (s_disp_panel == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    // Solo reinit via DBI (niente GPIO RST → touch non viene resettato)
    ESP_LOGW(TAG, "Panel recover: soft reinit");
    return esp_lcd_panel_init(s_disp_panel);
}
...
esp_codec_dev_handle_t bsp_audio_codec_microphone_init(void)
{
    ...
    if (es7210_dev == NULL) {
        ESP_LOGW("BSP", "ES7210 microphone not found on I2C, recording disabled");
        return NULL;
    }
...
{
    /* GPIO 23 è in conflitto con LEDC su questa board (warning "not usable").
     * Si usa GPIO semplice: HIGH = backlight ON. */
    ...
    BSP_ERROR_CHECK_RETURN_ERR(gpio_config(&bl_gpio_cfg));
    BSP_ERROR_CHECK_RETURN_ERR(gpio_set_level(BSP_LCD_BACKLIGHT, 0)); // parte spento
    return ESP_OK;
}
...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    /* Accende se brightness > 0, spegne se == 0 */
    return gpio_set_level(BSP_LCD_BACKLIGHT, brightness_percent > 0 ? 1 : 0);
}
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    s_disp_panel = disp_panel; // salva per recovery automatico
